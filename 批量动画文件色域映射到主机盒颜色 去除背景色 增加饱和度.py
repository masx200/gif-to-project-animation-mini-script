...
"""
    这段Python代码主要实现了一个功能：从输入的动画文件中读取像素的RGB值，如果某个像素的RGB值等于特定值（在此例中是（185, 194, 217）），那么这个像素会被修改为黑色，并保存到指定的输出目录中。
...
def color_transform(r, g, b):
    if r == g and g == b and b == r:
        return (0, 0, 0) if r < 128 else (255, 255, 255)
    min_value = min(r, g, b)
        ...
        with open(file, "r") as f:
            ...
            for line in data["anim"]:
                ...
                for i in range(0, len(values), 3):
                    ...
                    # 判断是否需要修改RGB值
                    # 修改RGB值并写入到输出目录中
                    if r == 185 and g == 194 and b == 217:
                        new_r, new_g, new_b = 0, 0, 0
                    else:
                        new_r = r
                        new_g = g
                        new_b = b
                        new_r, new_g, new_b = color_transform(
                            r,
                            g,
                            b,
                        )
